...
async function updateConf(conf: IConfig) {
  if (!conf) return
  await useNuxtApp().$PUT(`/config/update`, conf)
    ...
    table
      tbody
        ...
        tr.cursor-pointer(v-for="conf in spec.configs" @click="navigateTo(`/config/${conf.id}`)")
          ...
          td {{ conf.date }}
          td
            q-input(v-model="conf.count" type="number" :min="1" @update:model-value="updateConf(conf)" @click.stop="()=>{}")
          td
