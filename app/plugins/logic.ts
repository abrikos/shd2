    ...
    return {
        provide: {
            ...
            configValidator: (conf: IConfig) => {
                ...
                if(conf.platform.pcie8Max + conf.platform.pcie16Max < conf.pcie8Count){
                    list.push(`Недостаточно PCIE слотов (${conf.platform.pcie8Max + conf.platform.pcie16Max}) для выбранного количества устройств (${conf.pcie8Count})`);
                }
